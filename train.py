...
def checkpoint(model, epoch, save_path):
    ...
    model_out_path = f"./{save_path}/model_GPDL_iters_{epoch}.pth"
...
def train():
    ...
    print(f'===> Loading datasets with data_ratio: {data_ratio}')
    # Initialize dataset
    dataset = build_dataset(arg_dict, data_ratio=data_ratio)
    ...
    epoch_loss = 0
    iter_num = 0
    print_freq = len(dataset)
    save_freq = 5*print_freq
    arg_dict['max_iters'] = save_freq
        ...
        with tqdm(total=print_freq) as bar:
            for feature, label, _ in dataset:
                if arg_dict['cpu']:
                    ...
                    input, target = feature.cpu(), label.cpu()
                ...
                iter_num += 1
                all_losses.append(pixel_loss.item())
    ...
    plt.legend()
    plt.savefig(os.path.join(arg_dict['save_path'], 'model_GPDL_training_loss_curve.png'))
    plt.show()
